...
GO
CREATE PROC LOS_TRIGGERS.ComprarBonos(@afiliado numeric(18,0), @cantBonos numeric(4,0)) AS
	BEGIN
	SET IDENTITY_INSERT LOS_TRIGGERS.Compra_Bono ON
		...
		BEGIN
			...
			insert into LOS_TRIGGERS.Compra_Bono(comp_afiliado, comp_cantidad, comp_monto, comp_fecha_y_hora) 
				values ( @afiliado, @cantBonos,LOS_TRIGGERS.CalcularMontoCompraBonos(@afiliado, @cantBonos), GETDATE())
			SET @numeroCompra = @@IDENTITY
			SET IDENTITY_INSERT LOS_TRIGGERS.Compra_Bono OFF
			SET IDENTITY_INSERT LOS_TRIGGERS.Bono ON
			set @i = 1
			WHILE @i <= @cantBonos
			BEGIN
				insert into LOS_TRIGGERS.Bono(bono_compra, bono_plan_medico, bono_fecha_impresion)
					values(@numeroCompra, (select afil_plan_medico from LOS_TRIGGERS.Afiliado where afil_numero = @afiliado), GETDATE())
				set @i = @i+1
			END
		END
		ELSE PRINT 'Afiliado no se encuentra activo.'
